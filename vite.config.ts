    ...
    rollupOptions: {
      output: {
        manualChunks(id: string) {
          ...
          // These pages are statically imported in App.tsx so Vite adds
          // <link rel="modulepreload"> for them in the HTML <head>.
          // Browsers fetch them in parallel with the entry JS — eliminating
          // the React.lazy() network roundtrip that caused 3–5 s LCP delays.
          if (
            id.includes("/pages/OTTAdvertising") ||
            id.includes("/sections/ottAdvertising/")
          )
            return "page-ott";
          if (
            id.includes("/pages/WhiteLabelDSP") ||
            id.includes("/sections/whiteLabelDSP/")
          )
            return "page-wl-dsp";
          if (
            id.includes("/pages/CTVAdvertisingNew") ||
            id.includes("/sections/connectedTV/")
          )
            return "page-ctv";
          if (
