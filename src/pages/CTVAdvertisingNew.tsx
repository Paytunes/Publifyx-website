...
import { ArrowRight, Sparkles, Play, Tv, Target, BarChart3, Shield, RefreshCw, Zap, PhoneCall, Settings, Rocket, GraduationCap, Building2, Store, Megaphone } from "lucide-react";
import MagneticCard from "@/components/effects/MagneticCard";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion";

const features = [
  { title: "Premium Streaming Inventory", description: "Access ad-supported streaming platforms, FAST channels, and premium CTV inventory via SSP integrations.", icon: Tv },
  { title: "Precision Targeting", description: "Target households by interests, purchase intent, viewing behavior, income, and geography down to zip code.", icon: Target },
  { title: "Cross-Device Retargeting", description: "Retarget CTV viewers across mobile, desktop, and tablet with display or video follow-up campaigns.", icon: RefreshCw },
  { title: "Real-Time Reporting", description: "Monitor impressions, VCR, reach, frequency, and cost metrics with full transparency.", icon: BarChart3 },
  { title: "Brand Safety Controls", description: "Category-level and app-level exclusions plus third-party verification integrations.", icon: Shield },
  { title: "White Label CTV", description: "Agencies can offer CTV advertising as a branded service with custom reporting.", icon: Zap },
];

const steps = [
  { number: "01", title: "Define Objectives", description: "Set campaign goals — awareness, reach, performance, or brand lift.", icon: PhoneCall },
  ...
  { number: "03", title: "Upload & Launch", description: "Upload 15s or 30s HD video creatives and launch via real-time bidding.", icon: Rocket },
  { number: "04", title: "Monitor & Optimize", description: "Track VCR, reach, frequency, and cost — optimize in real time.", icon: GraduationCap },
];

const whoItsFor = [
  { title: "Brand Advertisers", description: "Deliver full-screen, non-skippable video ads on the biggest screen in the household with TV-level impact.", icon: Megaphone },
  { title: "Media Agencies", description: "Offer CTV as a premium channel within your programmatic stack with white-label branding.", icon: Building2 },
  { title: "Performance Marketers", description: "Combine CTV reach with cross-device retargeting for measurable, full-funnel outcomes.", icon: Target },
  { title: "Regional Businesses", description: "Target specific geographic areas with CTV ads — reach cord-cutters in your market at scale.", icon: Store },
];

const faqs = [
  { question: "What is the difference between CTV and OTT advertising?", answer: "CTV refers specifically to ads on TV screens via internet-connected devices. OTT covers all internet-delivered content across all devices. CTV is a subset of OTT." },
  { question: "What video formats are supported?", answer: "15-second and 30-second video ads in MP4 or MOV format, optimized for full-screen HD television viewing." },
  { question: "Can I target specific geographic areas?", answer: "Yes. Geographic targeting at country, state, city, DMA, and zip code level." },
  { question: "How is CTV advertising measured?", answer: "Impressions, VCR, unique household reach, frequency, CPCV, plus attribution for website visits and conversions." },
  { question: "Is CTV only for large brands?", answer: "No. CTV is accessible to all business sizes with flexible budgets through programmatic buying on PublifyX." },
];
...
const CTVAdvertising = () => {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute("content", "Launch data-driven CTV campaigns with PublifyX's connected TV advertising platform. Target households across premium streaming inventory.");
    ...
    <Layout transparentHeader>
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-navy-800">
        ...
        <div className="absolute bottom-1/4 left-1/4 w-80 h-80 bg-brand-orange-300 rounded-full blur-[160px] opacity-[0.06]" />
         <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <PageBreadcrumb items={[{ label: "Home", path: "/" }, { label: "Services", path: "/services" }, { label: "CTV Advertising" }]} />
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              ...
              <p className="text-lg md:text-xl text-navy-200 mb-4 leading-relaxed max-w-xl">
                Television has evolved. Connected TV advertising delivers full-screen, non-skippable video ads with the targeting precision of digital to viewers on smart TVs and streaming devices.
              </p>
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="inline-block text-sm font-semibold text-brand-orange-500 uppercase tracking-widest mb-3">CTV Advertising</motion.span>
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-4 max-w-4xl mx-auto">What Is CTV Advertising?</motion.h2>
            <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-lg text-navy-400 max-w-3xl mx-auto leading-relaxed">
              CTV advertising refers to video ads delivered through internet-connected television devices — smart TVs, Roku, Amazon Fire TV, Apple TV, and gaming consoles. Unlike linear TV, CTV ads are served programmatically to specific households based on data-driven targeting.
            </motion.p>
          </div>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            <div className="bg-navy-50 rounded-2xl border border-navy-100 p-8">
              ...
              <ul className="space-y-3">
                {["Smart TVs with built-in internet", "Roku, Fire TV, Apple TV", "PlayStation, Xbox consoles", "Internet-enabled set-top boxes"].map(item => (
            ...
            <div className="bg-navy-800 rounded-2xl border border-navy-700 p-8">
              ...
              <ul className="space-y-3">
                {["Immersive full-screen impact", "Non-skippable ad placements", "Household-level targeting", "Real-time optimization", "Transparent reporting"].map(item => (
                  <li key={item} className="flex items-center gap-3">
      ...
      <section className="py-20 md:py-28 bg-navy-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>CTV Advertising Platform Features</motion.h2>
          </div>
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Who Is CTV Advertising For?</motion.h2>
          </div>
      ...
      <section className="py-20 md:py-28 bg-navy-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>How CTV Advertising Works</motion.h2>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center">
            <Button onClick={handleGetStarted} className="bg-brand-orange-500 hover:bg-brand-orange-600 text-white px-10 py-6 rounded-xl font-semibold text-lg hover:scale-105 transition-all duration-300 shadow-lg shadow-brand-orange-500/25 group">
              Activate CTV Advertising <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 bg-navy-800 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <p className="text-xl text-navy-300 mb-4 leading-relaxed">Deliver non-skippable, full-screen video ads to viewers on smart TVs, Roku, Fire TV, Apple TV, and more.</p>
