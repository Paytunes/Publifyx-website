...
import { ArrowRight, Sparkles, Play, CheckCircle2, DollarSign, Layers, Palette, Network, GraduationCap, Headphones, RefreshCw, Building2, Globe, Star, Target } from "lucide-react";
import MagneticCard from "@/components/effects/MagneticCard";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion";

const included = [
  { title: "Platform Access", description: "Full DSP with campaign management, targeting, bidding, and reporting across all channels.", icon: Layers },
  { title: "White Label Branding", description: "Custom domain, logo, colors, and branded reporting at no additional charge.", icon: Palette },
  { title: "SSP Integrations", description: "Pre-configured connections to 40+ SSPs and ad exchanges. Custom integrations available.", icon: Network },
  { title: "Multi-Channel Access", description: "Display, video, audio, CTV, and OTT — all included in base pricing.", icon: DollarSign },
  { title: "Onboarding & Training", description: "Dedicated onboarding sessions, platform training, and documentation.", icon: GraduationCap },
  { title: "Technical Support", description: "Responsive support for platform issues, campaign troubleshooting, and optimization.", icon: Headphones },
  { title: "Updates & Maintenance", description: "Regular feature updates, security patches, and infrastructure maintenance included.", icon: RefreshCw },
];

const whoItsFor = [
  { title: "Growing Agencies", description: "Flexible pricing that scales with your client base. Start lean, grow without constraints.", icon: Building2 },
  { title: "Ad Networks", description: "Predictable costs for offering self-serve programmatic to your advertiser base.", icon: Globe },
  { title: "Startups", description: "Enterprise-grade technology at a fraction of the build cost. Launch without millions in development.", icon: Star },
  { title: "Performance Teams", description: "Transparent pricing with no hidden markups — your margins improve as you grow.", icon: Target },
];

const faqs = [
  { question: "How much does a white label DSP cost?", answer: "Pricing varies based on features, channels, and media spend volume. PublifyX offers flexible plans. Contact our team for a custom quote." },
  { question: "Does PublifyX charge a percentage of media spend?", answer: "PublifyX structures pricing to remain cost-effective as your business scales. Your margins improve with growth. Contact us for details." },
  { question: "Are there setup fees?", answer: "Setup requirements vary by deployment. White label branding, domain setup, and standard SSP integrations are included." },
  { question: "Is it cheaper to build or buy a DSP?", answer: "Building from scratch costs millions and requires dedicated engineering. A white label DSP provides the same functionality at a fraction of the cost in days." },
  { question: "Can I upgrade my plan?", answer: "Yes. Plans scale with your business. As media spend and client base grow, your plan adjusts accordingly." },
];
...
const Pricing = () => {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute("content", "Explore PublifyX white label DSP pricing. Flexible plans for agencies, ad networks, and startups. Transparent costs with no hidden fees.");
  ...
  return (
    <Layout transparentHeader>
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-navy-800">
        ...
        <div className="absolute bottom-1/4 left-1/4 w-80 h-80 bg-brand-orange-300 rounded-full blur-[160px] opacity-[0.06]" />
         <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <PageBreadcrumb items={[{ label: "Home", path: "/" }, { label: "Resources", path: "/resources" }, { label: "Pricing" }]} />
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              ...
              <p className="text-lg md:text-xl text-navy-200 mb-4 leading-relaxed max-w-xl">
                Enterprise-grade programmatic technology without the enterprise-grade price tag. No hidden fees, no surprise charges, no percentage-based markups.
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start mb-10">
                <Button onClick={handleGetStarted} className="bg-brand-orange-500 hover:bg-brand-orange-600 text-white px-10 py-6 rounded-xl font-semibold text-lg transition-all duration-300 hover:scale-105 shadow-lg shadow-brand-orange-500/25 group">
                  Request Custom Quote <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </Button>
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="inline-block text-sm font-semibold text-brand-orange-500 uppercase tracking-widest mb-3">What's Included</motion.span>
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Everything in Every Plan</motion.h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {included.map((item, i) => (
              <motion.div key={item.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className={i === included.length - 1 ? "md:col-span-2 lg:col-span-1" : ""}>
      ...
      <section className="py-20 md:py-28 bg-navy-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="inline-block text-sm font-semibold text-brand-orange-500 uppercase tracking-widest mb-3">Pricing Advantage</motion.span>
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Why PublifyX Pricing Makes Sense</motion.h2>
          </div>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            <div className="bg-white rounded-2xl border border-navy-100 p-8">
              ...
              <ul className="space-y-3">
                {["Percentage of media spend (gets expensive)", "Per-channel premiums", "Per-integration fees", "Hidden markups on media costs"].map(item => (
            ...
            <div className="bg-navy-800 rounded-2xl border border-navy-700 p-8">
              ...
              <ul className="space-y-3">
                {["Predictable platform access fee", "All channels included", "All integrations included", "Branding included at no extra cost", "Margins improve as you grow"].map(item => (
                  <li key={item} className="flex items-center gap-3">
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Pricing for Every Business Stage</motion.h2>
          </div>
      ...
      <section className="py-20 md:py-28 bg-navy-800 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <p className="text-xl text-navy-300 mb-4 leading-relaxed">Every business has unique requirements. Get pricing tailored to your goals.</p>
