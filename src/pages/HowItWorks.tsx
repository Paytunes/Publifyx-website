...
import { ArrowRight, Sparkles, Play, Rocket, GraduationCap, Search, Palette, Network, Building2, Globe, Star, Target } from "lucide-react";
import MagneticCard from "@/components/effects/MagneticCard";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion";

const steps = [
  { number: "01", title: "Discovery & Consultation", description: "We discuss your business model, target markets, channels to activate, client access model, branding specs, and SSP integration needs.", icon: Search },
  { number: "02", title: "Platform Configuration", description: "Our team configures your custom domain, logo, color palette, account structure, user roles, and permission levels.", icon: Palette },
  { number: "03", title: "Supply Integration", description: "We activate connections to 40+ SSPs covering display, video, audio, CTV, and OTT — plus any custom supply partners.", icon: Network },
  { number: "04", title: "Team Onboarding", description: "Comprehensive training on platform navigation, campaign setup, targeting, bidding, reporting, and optimization.", icon: GraduationCap },
  { number: "05", title: "Campaign Launch", description: "Go live with your first campaigns. Our support team assists with strategy, troubleshooting, and optimization.", icon: Rocket },
];

const whoItsFor = [
  { title: "Media Agencies", description: "Deploy a branded DSP with full white-label customization. Control margins, client access, and reporting.", icon: Building2 },
  { title: "Ad Networks", description: "Add self-serve programmatic buying capabilities for your advertiser base — fast and scalable.", icon: Globe },
  { title: "Startups", description: "Enter the ad tech market without millions in development costs. Enterprise-grade technology, ready in days.", icon: Star },
  { title: "Performance Marketers", description: "Launch data-driven campaigns across multiple channels with advanced targeting and real-time optimization.", icon: Target },
];

const faqs = [
  { question: "How long does the full setup process take?", answer: "Most deployments complete within five to ten business days, depending on customization and integration requirements." },
  { question: "Do I need engineering resources?", answer: "No. PublifyX handles all technical setup, configuration, and ongoing platform maintenance. Your team focuses on business operations." },
...
const HowItWorks = () => {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute("content", "Learn how PublifyX works — from initial setup to live campaign launch. Deploy your white label DSP in days.");
    ...
    <Layout transparentHeader>
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-navy-800">
        ...
        <div className="absolute bottom-1/4 left-1/4 w-80 h-80 bg-brand-orange-300 rounded-full blur-[160px] opacity-[0.06]" />
         <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <PageBreadcrumb items={[{ label: "Home", path: "/" }, { label: "Resources", path: "/resources" }, { label: "How It Works" }]} />
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              ...
              <p className="text-lg md:text-xl text-navy-200 mb-4 leading-relaxed max-w-xl">
                Launching your own DSP sounds complex. With PublifyX, we handle the technology, infrastructure, and integrations — you focus on your brand, clients, and growth.
              </p>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.3 }} className="hidden lg:grid grid-cols-2 gap-4">
              {[
                ...
                { value: "Zero", label: "Engineering Required" },
              ].map((stat, i) => (
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="inline-block text-sm font-semibold text-brand-orange-500 uppercase tracking-widest mb-3">The Process</motion.span>
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Five Steps to Your Own DSP</motion.h2>
            <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-lg text-navy-400 max-w-2xl mx-auto mt-4">From consultation to live campaigns — typically within five to ten business days.</motion.p>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mt-12">
            <Button onClick={handleGetStarted} className="bg-brand-orange-500 hover:bg-brand-orange-600 text-white px-10 py-6 rounded-xl font-semibold text-lg hover:scale-105 transition-all duration-300 shadow-lg shadow-brand-orange-500/25 group">
              Start Step 1 — Book a Call <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 bg-navy-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Who Is This Process For?</motion.h2>
          </div>
      ...
      <section className="py-20 md:py-28 bg-navy-800 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <p className="text-xl text-navy-300 mb-4 leading-relaxed">Most clients go from consultation to live campaigns within five to ten business days.</p>
          <p className="text-base text-navy-400 mb-10 font-semibold">No commitment required · Zero engineering needed</p>
