...
const ProgrammaticAudioAdvertising = () => {
  ...
  return (
    <ServicePageLayout
      ...
      heroSubtitle="Audio advertising is one of the fastest-growing channels in programmatic media buying. Reach audiences during screen-free moments — while commuting, exercising, cooking, or working."
    ...
    >
      <section className="space-y-12">
        <div>
          ...
          <p className="text-navy-600 leading-relaxed mb-4">Programmatic audio advertising is the automated buying and selling of audio ad inventory using real-time bidding technology. When a listener streams music, a podcast, or digital radio content, an audio ad slot becomes available. A bid request is sent to demand-side platforms, where advertisers compete in a real-time auction based on targeting criteria.</p>
          <p className="text-navy-600 leading-relaxed">This process happens in milliseconds and eliminates the need for manual negotiations with individual publishers. Programmatic audio ads are typically 15 or 30 seconds in length and are often accompanied by a companion display banner.</p>
        </div>
        ...
        <div>
          ...
          <p className="text-navy-600 leading-relaxed mb-4">Audio reaches audiences in moments where visual media cannot. Listeners engage with audio content during activities like driving, exercising, and working — moments when they are attentive but away from screens.</p>
          <p className="text-navy-600 leading-relaxed">Audio ads benefit from high completion rates. Unlike display ads that can be scrolled past or video ads that can be skipped, audio ads are typically played in full. Digital audio consumption continues to grow globally, driven by the expansion of streaming music services, the podcasting boom, and the proliferation of smart speakers.</p>
        </div>

        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-navy-900 mb-6">Platform Features</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { title: "Cross-Platform Audio Inventory", desc: "Access audio ad inventory across streaming music services, podcast networks, digital radio stations, and smart speaker environments." },
              { title: "Precision Listener Targeting", desc: "Target audio audiences by demographics, geographic location, interests, listening behavior, content genre, device type, and time of day." },
              { title: "Dynamic Creative Capabilities", desc: "Serve different audio creatives to different audience segments based on targeting criteria for personalized messaging at scale." },
              { title: "Companion Display Banners", desc: "Pair your audio ad with a visual companion banner that appears in the listener's app for reinforcement." },
              { title: "Real-Time Reporting", desc: "Track performance with impressions, completion rate, reach, frequency, and cost per completed listen." },
              { title: "Unified Platform", desc: "Manage audio campaigns alongside display, video, CTV, and OTT within PublifyX — no separate tools required." },
            ].map((item, i) => (
        ...
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-navy-900 mb-6">Audio Ad Formats</h2>
          <div className="space-y-4">
            {[
              ...
              { title: "Podcast Ads with Dynamic Insertion", desc: "Targeted audio ads dynamically inserted into podcast episodes at listen time, based on the listener's profile." },
              { title: "Companion Display Banners", desc: "Visual banners that appear within the streaming app while the audio ad plays, providing a clickable call-to-action." },
              { title: "Sequential Audio Messaging", desc: "Deliver a series of audio ads to the same listener over time, building a narrative or guiding them through a funnel." },
            ].map((item, i) => (
