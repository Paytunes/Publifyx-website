...
import { ArrowRight, Sparkles, Play, Headphones, Radio, Target, BarChart3, Mic, Zap, RefreshCw, Globe, PhoneCall, Settings, Rocket, GraduationCap, Building2, Store, Megaphone, Music } from "lucide-react";
import MagneticCard from "@/components/effects/MagneticCard";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion";

const features = [
  { title: "Cross-Platform Audio", description: "Access inventory across streaming music, podcast networks, digital radio, and smart speakers.", icon: Globe },
  { title: "Precision Targeting", description: "Target by demographics, geography, interests, listening behavior, content genre, and time of day.", icon: Target },
  { title: "Dynamic Creative", description: "Serve different audio creatives to different segments based on targeting criteria for personalized messaging.", icon: Mic },
  { title: "Companion Banners", description: "Pair audio ads with visual companion banners in the listener's app for reinforced messaging.", icon: Radio },
  { title: "Real-Time Reporting", description: "Track impressions, completion rate, reach, frequency, and cost per completed listen.", icon: BarChart3 },
  { title: "Unified Platform", description: "Manage audio alongside display, video, CTV, and OTT — no separate tools or logins required.", icon: Zap },
];

const adFormats = [
  { title: "In-Stream Audio Ads", desc: "15 or 30-second spots before, during, or between songs, podcasts, or radio content." },
  { title: "Podcast Dynamic Insertion", desc: "Targeted audio ads dynamically inserted into podcast episodes at listen time." },
  { title: "Companion Display Banners", desc: "Visual banners appearing in the streaming app while the audio ad plays." },
  { title: "Sequential Audio Messaging", desc: "A series of audio ads delivered to the same listener over time for narrative storytelling." },
];

const steps = [
  { number: "01", title: "Upload Audio Creatives", description: "Upload your 15s or 30s audio ad creatives with optional companion banners.", icon: PhoneCall },
  { number: "02", title: "Set Targeting", description: "Define audiences by demographics, listening behavior, genre, and geography.", icon: Settings },
  { number: "03", title: "Launch Campaigns", description: "Go live across audio inventory via programmatic buying.", icon: Rocket },
  { number: "04", title: "Optimize & Report", description: "Monitor completion rates, reach, and frequency — optimize in real time.", icon: GraduationCap },
];

const whoItsFor = [
  { title: "Brand Advertisers", description: "Reach audiences during screen-free moments — commuting, exercising, cooking — when visual ads can't.", icon: Megaphone },
  { title: "Media Agencies", description: "Add audio as a premium channel to your programmatic stack with unified reporting.", icon: Building2 },
  { title: "Podcast Advertisers", description: "Dynamically insert targeted ads into podcast episodes with precision and measurability.", icon: Music },
  { title: "Local Businesses", description: "Target specific geographic areas with audio ads on local streaming and radio platforms.", icon: Store },
];

const faqs = [
  { question: "What is programmatic audio advertising?", answer: "Programmatic audio uses automated, real-time bidding to buy and deliver audio ads across streaming music, podcasts, and digital radio." },
  { question: "What platforms do audio ads run on?", answer: "Streaming music services, podcast networks, digital radio platforms, and smart speaker environments." },
  { question: "How long are audio ads?", answer: "Standard lengths are 15 and 30 seconds. Some platforms support 6-second or 60-second formats." },
  { question: "Can I target specific audiences?", answer: "Yes. Targeting by demographics, geography, interests, listening behavior, content genre, device type, and time of day." },
  { question: "How is performance measured?", answer: "Key metrics: impressions, audio completion rate, unique listener reach, frequency, and cost per completed listen." },
];
...
const ProgrammaticAudioAdvertising = () => {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute("content", "Run targeted audio ads on streaming music, podcasts, and digital radio with PublifyX's programmatic audio platform.");
    ...
    <Layout transparentHeader>
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-navy-800">
        ...
        <div className="absolute bottom-1/4 left-1/4 w-80 h-80 bg-brand-orange-300 rounded-full blur-[160px] opacity-[0.06]" />
         <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <PageBreadcrumb items={[{ label: "Home", path: "/" }, { label: "Services", path: "/services" }, { label: "Programmatic Audio Advertising" }]} />
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              ...
              <p className="text-lg md:text-xl text-navy-200 mb-4 leading-relaxed max-w-xl">
                Audio reaches audiences during screen-free moments — commuting, exercising, cooking, and working. The fastest-growing channel in programmatic.
              </p>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.3 }} className="hidden lg:grid grid-cols-2 gap-4">
              {[
                { value: "95%+", label: "Audio Completion Rate" },
                { value: "40+", label: "SSP Integrations" },
                { value: "4", label: "Audio Ad Formats" },
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="inline-block text-sm font-semibold text-brand-orange-500 uppercase tracking-widest mb-3">Audio Advertising</motion.span>
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-4 max-w-4xl mx-auto">Why Audio Advertising?</motion.h2>
            <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-lg text-navy-400 max-w-3xl mx-auto leading-relaxed">
              Audio reaches audiences in moments where visual media cannot. Listeners are attentive but away from screens. Audio ads benefit from high completion rates — unlike display or video, audio ads are typically played in full.
            </motion.p>
          </div>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            <div className="bg-navy-50 rounded-2xl border border-navy-100 p-8">
              ...
              <ul className="space-y-3">
                {["Screen-free audience reach", "95%+ completion rates", "Growing listener base globally", "Podcast boom driving inventory"].map(item => (
            ...
            <div className="bg-navy-800 rounded-2xl border border-navy-700 p-8">
              ...
              <ul className="space-y-3">
                {["Automated real-time bidding", "Demographic & behavioral targeting", "Dynamic creative optimization", "Cross-channel integration", "Measurable attribution"].map(item => (
                  <li key={item} className="flex items-center gap-3">
      ...
      <section className="py-20 md:py-28 bg-navy-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Audio Platform Features</motion.h2>
          </div>
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Audio Ad Formats Supported</motion.h2>
          </div>
      ...
      <section className="py-20 md:py-28 bg-navy-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Who Is Audio Advertising For?</motion.h2>
          </div>
          <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center">
            <Button onClick={handleGetStarted} className="bg-brand-orange-500 hover:bg-brand-orange-600 text-white px-10 py-6 rounded-xl font-semibold text-lg hover:scale-105 transition-all duration-300 shadow-lg shadow-brand-orange-500/25 group">
              Start Your Audio Campaign <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 bg-navy-800 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <p className="text-xl text-navy-300 mb-4 leading-relaxed">Reach listeners across streaming music, podcasts, and digital radio with precision targeting.</p>
